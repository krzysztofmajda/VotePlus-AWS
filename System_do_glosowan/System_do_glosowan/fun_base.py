...
def get_email_and_username_for_edit_user(data=''):
    cur = mysql.connection.cursor()
    if data != '':
        cur.execute('SELECT user_id, `e-mail`, username FROM users WHERE (`e-mail` = (%s) OR username = (%s)) AND user_id NOT IN (SELECT user_id FROM inactive_user)',(data,data))
    else:
        cur.execute('SELECT user_id, `e-mail`, username FROM users WHERE user_id != 0 AND user_id NOT IN (SELECT user_id FROM inactive_user)')
...
def get_full_info_user(pid):
    cur = mysql.connection.cursor()
    cur.execute('SELECT users.name, users.surname, users.sex, users.birth_year,users.username,users.`e-mail`,user_role.role_name FROM users LEFT JOIN user_role ON users.user_id=user_role.user_id WHERE users.user_id = (%s)',(pid,))
    data = cur.fetchone()
    cur.close()
    return data

def update_user(pid, name, surname, role, sex, year, changed):
    con = mysql.connection
    con.autocommit = False
    cur = con.cursor()
    ...
    cur.execute('SELECT * FROM user_role WHERE user_id=(%s)',(pid,))
    tmp_role = cur.fetchone()
    
    cur.execute('UPDATE users SET name=(%s), surname=(%s), sex=(%s), birth_year=(%s) WHERE user_id=(%s)',(name,surname,sex,year,pid))
    
    if role == 'Tak' and tmp_role is None:
        cur.execute('INSERT INTO user_role VALUES(%s, %s, %s)',(max+1,'Edytor',pid))
    elif role == 'Nie' and not tmp_role is None:
        cur.execute('DELETE FROM user_role WHERE user_id=(%s)',(pid,))
    
    con.commit()

    if changed == 1:
        cur.execute('SELECT username, `e-mail` FROM users WHERE user_id=(%s)',(pid,))
        name_email = cur.fetchone()
        cur.close()
        fun_mail.edit_user(name, surname, role, sex, str(year), name_email[0], name_email[1])
    else:
        cur.close()
    return 0
...
def delete_user(pid):
    ...
    cur.execute('DELETE FROM user_role WHERE user_id = (%s)',(pid,))
    cur.execute('DELETE FROM user_group WHERE user_id = (%s)',(pid,))
    cur.execute('DELETE FROM vote WHERE user_id = (%s)',(pid,))
    cur.execute('DELETE FROM password WHERE user_id = (%s)',(pid,))
    cur.execute('UPDATE poll SET creator=NULL WHERE creator = (%s)',(pid,))
    cur.execute('DELETE FROM users WHERE user_id = (%s)',(pid,))
    con.commit()
    cur.close()
...
def create_poll(title, description, poll_type, start, end, creator):
    ...
    cur.execute('SELECT MAX(poll_id) FROM poll')
    id_max = cur.fetchone()[0]+1

    group_name = ("Grupa dla głosowania: " + title)
    cur.execute('SELECT COUNT(*) FROM `group` WHERE name LIKE (%s)',(("%"+group_name+"%"),))
    name_count = cur.fetchone()[0]
    group_name = (group_name + " " + str(name_count))
...
def get_full_name_for_add_to_poll(data=''):
    cur = mysql.connection.cursor()
    if data != '':
        cur.execute('SELECT user_id, name, surname, username FROM users WHERE username = (%s) AND user_id NOT IN (SELECT user_id FROM inactive_user)',(data,))
    else:
        cur.execute('SELECT user_id, name, surname, username FROM users WHERE user_id != 0 AND user_id NOT IN (SELECT user_id FROM inactive_user)')
...
def get_group_for_select(current_group):
    cur = mysql.connection.cursor()
...
def get_users_full_name_from_group(group_id):
    cur = mysql.connection.cursor()
    cur.execute('SELECT users.name, users.surname, users.username FROM users INNER JOIN user_group ON user_group.user_id=users.user_id WHERE user_group.group_id=(%s)',(group_id,))
    users = cur.fetchall()
    cur.close()
    if users is None:
        users = []
...
def get_polls_for_edit(pid, name=''):
    cur = mysql.connection.cursor()
    ...
    if pid == 0:
        ...
        if name == '':
            cur.execute('SELECT poll_id, title, start_datetime FROM poll WHERE poll_id != 0 AND creator=(%s) AND start_datetime>(%s)',(pid,time))
        else:
            cur.execute('SELECT poll_id, title, start_datetime FROM poll WHERE poll_id != 0 AND creator=(%s) AND title LIKE (%s) AND start_datetime>(%s)',(pid,"%"+name+"%",time))
    polls = cur.fetchall()
    if polls is None:
        polls = []
    cur.close()
...
def polls_for_user_to_vote(pid):
    cur = mysql.connection.cursor()
    time = fun.generate_current_datetime()
    cur.execute('SELECT poll.poll_id, poll.title, poll.end_datetime FROM poll INNER JOIN user_group ON poll.group_id=user_group.group_id WHERE user_group.user_id=(%s) AND user_group.if_voted=\'0\' AND poll.start_datetime<=(%s) AND poll.end_datetime>=(%s)',(pid,time,time))
    polls = cur.fetchall()
    print(polls)
    cur.close()
    return polls
...
def get_poll_full_info(poll_id):
    poll_questions_with_answers = []
    cur = mysql.connection.cursor()
    cur.execute('SELECT `title`,`type`,`description`,`start_datetime`,`end_datetime` FROM poll WHERE poll_id=(%s)',(poll_id,))
    info = cur.fetchone()
    return info
